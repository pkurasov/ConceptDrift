    ...
    public static void main(String[] args) {
        try {
            if (args.length < 1) {
                ...
                // parse options
                AbstractTask task;
                //EvaluatePrequential ep;
                try {
                	LOGGER.debug("calling ClassOption.cliStringToObject with "
                			+ "cliString: " + cliString.toString()
                			+'\n'+ "MainTask.class:" + MainTask.class.getName()
                			+'\n'+ "extraOptions:" + Arrays.toString(extraOptions)+'\n'
                			+'\n'+ "Classs of ClassOption.cliStringToObject " + 
                			"(cliString.toString(), MainTask.class, extraOptions)" + 
                			" before casting" +'\t'+ (ClassOption.cliStringToObject (cliString.toString(), MainTask.class, extraOptions)).getClass().toString() +'\n'
                			+ " What is a superclass of above var before casting?:  "
                			+ ((ClassOption.cliStringToObject (cliString.toString(), MainTask.class, extraOptions)).getClass().getSuperclass())+'\n'
                			+ "status that would be assigned to this class if it were to be initialized  "  
                			+ Class.forName((ClassOption.cliStringToObject (cliString.toString(), MainTask.class, extraOptions)).toString(), true, null).desiredAssertionStatus());
                ...
                if (result instanceof FailedTaskReport) {
                    ...
                    if (!suppressResultOutputOption.isSet()) {
                        if (result instanceof Measurement[]) {
                        	...
                        	/*String myResult= "learning evaluation instances,evaluation time (cpu seconds),"
                        			+ "model cost (RAM-Hours),classified instances,classifications correct (percent),"
                        			+ "Kappa Statistic (percent),Kappa Temporal Statistic (percent),Kappa M Statistic (percent),"
                            ...
                            String temp = null;
                            while (scan.hasNext()) { 
                      		  temp = scan.next().strip();
                      		System.out.println(scan.nextLine());
                      		System.out.println(scan.hasNext());
                            	
                      			myResult = scan.nextLine();
                            ...
                            File file = new File("NumericOutput.csv");
                        	if(!file.exists()) {
                        		file.createNewFile();
                        	}
                        	
                        	try( FileWriter out = new FileWriter("NumericOutput.csv", true); 
